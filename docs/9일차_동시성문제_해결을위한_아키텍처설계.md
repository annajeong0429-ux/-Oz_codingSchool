...
3. 이 구조의 핵심 부품이 **Redis(작업 대기열 + 결과 Pub/Sub)** 이며, **최종 분석 결과는 MySQL `ai_analysis_results` 테이블에 영구저장(+캐싱)** 한다. 구현은 과제 stage3 요구대로 **raw Redis(List 큐 + Pub/Sub)를 직접 작성**하고(Celery·Redis Streams는 검토한 대안), 결과는 **FastAPI가 subscribe해서 응답으로 반환**한다.
...
### 2-4. 결과 전달 방식 — subscribe-return (채택) + 폴링/SSE (대안)

워커가 "언제 끝났는지"는 워커만 안다. FastAPI가 그 결과를 클라이언트에 전하는 방식이 몇 가지다.

**① subscribe-return — ✅ 우리 채택 (과제 stage3 요구)**
- FastAPI가 작업을 큐에 넣은 뒤 **결과 채널(`task:{id}`)을 직접 subscribe** → 워커가 publish할 때까지 기다렸다가 **그 응답에 결과를 담아 반환**.
- 클라이언트는 **요청 1번 → 결과 1번** (폴링·별도 알림 불필요).
- 무거운 추론은 **워커가** 하므로 FastAPI 이벤트 루프는 안 막히고 여러 요청을 동시에 await 가능.
- ⚠️ **순서 주의**: pub/sub은 저장이 안 돼서 **enqueue 전에 subscribe 먼저** 해야 결과를 안 놓친다.

> **NFR 트레이드오프(정직하게)**: subscribe-return은 **요청을 결과까지 들고 있어** 예측 엔드포인트는 "3초 응답"을 완전히는 못 지킨다(추론 끝까지 대기). 대신 **무거운 추론을 워커로 분리**(이벤트 루프 보호 + 워커 확장)라는 동시성 목표는 달성. → **기본 구현은 결과까지 대기**(subscribe 타임아웃 **30s**, §7-2 #9 — 추론이 보통 몇 초라 그 안에 받는다). 엄격한 3초가 필요하면 **3초 타임아웃 후 `202 + task_id` 폴백**으로 확장(선택).

**② 폴링(polling) — 대안**
- 클라가 `GET /predictions/{task_id}` 로 주기 조회. 진짜 "202 즉시 응답"이 필요할 때.
- ⚠️ **폴링 스톰**: 자주 물으면 조회가 쌓임. 단 폴은 Redis/DB 조회(수 ms)라 가볍고 워커는 안 막음. 완화: 간격(2~3초)+지수 백오프.

**③ SSE 푸시 — 대안(향후 개선)**
- 워커가 완료를 Pub/Sub로 publish → FastAPI가 subscribe → **SSE로 클라에 push**. 폴링 스톰 근본 제거 + 실시간. (WebSocket보다 단순한 단방향 HTTP, 의존성 `sse-starlette`)

> 💡 **공통 뼈대**: 셋 다 "워커가 결과를 Pub/Sub로 알린다"는 같다. **차이는 FastAPI가 그걸 ①요청 응답에 바로 / ②클라 폴링 / ③SSE push 중 무엇으로 전하느냐**. 우리는 과제대로 **①subscribe-return** 채택. (아래 코드 골격은 ③ SSE 변형 예시)
...
**구현 순서(권장)**: stage3는 **①subscribe-return으로 먼저** 동작시켜 과제 완료조건(요청→큐→워커→pub/sub→DB→응답)을 충족한다. 엄격한 3초 NFR·실시간이 필요해지면 ②폴링/③SSE를 얹는다. (갈아끼우는 구조라 리스크 낮음)
...
> **결론**: 아래 Celery·Redis Streams는 **검토한 방안**(EDA 원리 학습용). **실제 구현은 과제 stage3 요구에 따라 raw Redis(List 큐 + Pub/Sub)를 직접 작성**한다 — 최종 채택·계약은 §6·§7-2 참고.
...
| **우리 적합도** | 📘 검토(이번 과제 범위 밖) | 📘 개념 학습용 |

**최종 채택 (stage3)**
- ✅ **raw Redis (List 큐 + Pub/Sub) 직접 구현** — 과제 stage3가 `redis_client`(async/sync) 직접 작성을 요구. FastAPI는 결과를 **subscribe-return**으로 받아 DB 저장·응답한다. (인터페이스 계약 = §7-2)
- **Celery / Redis Streams**는 위에서 **검토한 대안** — Celery는 재시도·모니터링이 내장이라 추후 규모 커지면 후보, Streams는 EDA 원리(consumer group·ack) 학습용.

---

## 7. Docker Compose 구성 (Stage 1 → Stage 3 연계)

Stage 1에서 만든 `fastapi` + `mysql` 에 **`redis` + `ai-worker`** 를 추가한다. **과제 stage3 요구대로 이미지 용량 최소화를 위해 worker는 별도 `worker/Dockerfile`(ai 의존성=torch만)로 분리**하고(fastapi `app/Dockerfile`은 torch 제외 → 경량), **X-ray 파일 공유를 위해 `media_volume`을 함께 마운트**한다.
...
services:
  fastapi:                                   # 생산자(Producer)
    build:
      context: .
      dockerfile: app/Dockerfile             # app(web) 의존성 — torch 제외 → 경량
    ...
    environment:
      - DB_HOST=mysql
      - REDIS_URL=redis://redis:6379/0       # raw Redis (Celery 아님)
    depends_on:                              # ★ healthy 후 시작 (리뷰 반영)
      mysql:
        condition: service_healthy
      redis:
        condition: service_healthy

  ai-worker:                                 # 소비자(Consumer) — 별도 이미지(ai 의존성)
    build:
      context: .
      dockerfile: worker/Dockerfile          # ★ ai 스택(torch 등)만 → 용량 분리
    command: python -m worker.main           # 큐 BRPOP → 추론 → 결과 PUBLISH
    volumes:
      - .:/app
      - /app/.venv                           # 컨테이너 ai-venv 보호
      - media_volume:/app/media              # ★ api가 저장한 X-ray를 worker가 읽음
    env_file: [.env]
    environment:
      - REDIS_URL=redis://redis:6379/0
    depends_on:                              # worker는 DB 접속 안 함 → redis만
      redis:
        condition: service_healthy

  redis:                                     # 작업 큐(List) + 결과 Pub/Sub (공식 이미지)
    image: redis:7-alpine
    volumes:
      - redis_volume:/data
    healthcheck:                             # ★ condition용 (리뷰 반영)
...
> 포인트 정리
> - **fastapi / ai-worker** = **별도 Dockerfile + 의존성 분리**(과제 stage3 요구). fastapi=`app/Dockerfile`(web 스택, torch 제외 → 경량), worker=`worker/Dockerfile`(ai 스택=torch). pyproject에 app/ai 의존성 그룹 분리.
> - **redis / mysql** = 공식 이미지(빌드 X), 각각 healthcheck → `depends_on: condition: service_healthy`로 준비된 뒤 시작.
> - **`media_volume` 공유**가 핵심 — X-ray는 파일이라, API가 저장한 걸 Worker가 같은 볼륨에서 읽는다.
> - **worker는 DB 접속 안 함** — 추론 결과를 Pub/Sub로 publish만 하고, **DB 저장은 FastAPI**가 한다(§7-2 #11). 그래서 worker는 mysql 의존이 없다.
> - **다중 워커**: `docker compose up --scale ai-worker=3` — 여러 워커가 같은 List 큐에서 `BRPOP`로 작업을 나눠 가져간다.

---

## 7-2. Redis 작업 큐 인터페이스 계약 (stage3 확정)

생산자(FastAPI) ↔ 소비자(Worker)가 Redis로 주고받는 규칙. **구현 전 팀 합의 완료.**

| # | 항목 | 값 |
|---|---|---|
| 1 | 메커니즘 | **raw Redis** — 큐 = List(`LPUSH`/`BRPOP`), 결과 = Pub/Sub |
| 2 | 큐 이름 | `predictions` |
| 3 | task_id | FastAPI가 `uuid4()` 생성 |
| 4 | 큐 메시지 | `{"task_id","record_id","image_path"}` (image_path=상대경로) |
| 5 | 결과 채널 | `task:{task_id}` (워커 `PUBLISH` / FastAPI `SUBSCRIBE`) |
| 6 | 결과(성공) | `{"status":"completed","is_pneumonia","confidence","heatmap_url","model_name"}` |
| 7 | 결과(실패) | `{"status":"failed","error":"..."}` |
| 8 | FastAPI 수신 | **enqueue 전 SUBSCRIBE** → 결과 대기(타임아웃 30s) → DB 저장 → 응답 |
| 9 | 타임아웃 시 | `504` (또는 `202 + task_id` 폴백) |
| 10 | 캐싱(필수) | 큐 등록 **전** `(record_id, MODEL_NAME)` DB 확인 → 있으면 즉시 `200` |
| 11 | DB 저장 | **FastAPI**가 결과 받아 `ai_analysis_results` 저장 (워커는 publish만) |
| 12 | Redis 연결 | `REDIS_URL=redis://redis:6379/0` (env, compose 서비스명 `redis`) |
| 13 | X-ray 공유 | `media_volume`을 **fastapi·worker 둘 다** 마운트 (워커가 image_path로 읽음) |

- **담당**: 생산자(FastAPI)=ran / 소비자(worker)=정인 / 공유 인프라(compose·pyproject 의존성 분리)=ran 총괄
- **⚠️ subscribe-return의 약점**: 워커가 작업 처리 중 죽으면 publish가 없어 → FastAPI 타임아웃(504) → **결과가 DB에 안 남는다**(#11). `BRPOP`은 큐에서 꺼내는 순간 작업이 사라지므로 재처리도 불가.
- **선택(시간 되면, 위 약점까지 커버)**:
  - **워커 크래시 복구** = `BRPOPLPUSH`로 작업을 "처리중 리스트"에 옮겨두고 완료 시 제거 → 죽으면 미완료 작업이 처리중 리스트에 남아 **재처리 가능**(결과 유실 방지).
  - **중복(따닥) 방지** = `SETNX task_lock:{record_id}` 로 같은 record 동시요청 1건만 처리.
...
동시성 문제의 본질은 "느린 작업(AI 추론)을 빠른 응답 경로에 그대로 묶어두는 것"이다. 우리 프로젝트는 현재 `run_in_threadpool`로 이벤트 루프는 안 막지만 **요청 자체는 추론이 끝날 때까지 기다리고, 스레드풀이 포화되면 동시성이 무너진다.** 이벤트 기반 아키텍처는 **생산(FastAPI) → 큐(Redis) → 처리(Worker)** 로 단계를 분리해, **무거운 추론을 별도 워커가** 처리하고 FastAPI는 결과를 **Pub/Sub로 subscribe해 응답**한다(stage3 채택 = raw Redis). **Redis는 작업 대기열(List) + 결과 Pub/Sub**을, **MySQL `ai_analysis_results`는 최종 결과의 영구저장·캐싱**을, **`media_volume`은 X-ray 파일 공유**를 담당한다. 이 구조 덕분에 폐렴 예측 서비스는 무거운 AI 추론을 **API 본체와 분리**해(이벤트 루프 보호·워커 확장) 안정적으로 수행한다. (엄격한 3초 응답이 필요하면 `202 + 폴링/SSE`로 확장.)
