...
> **NFR 트레이드오프(정직하게)**: subscribe-return은 **요청을 결과까지 들고 있어** 예측 엔드포인트는 "3초 응답"을 완전히는 못 지킨다(추론 끝까지 대기). 대신 **무거운 추론을 워커로 분리**(이벤트 루프 보호 + 워커 확장)라는 동시성 목표는 달성. → **절충**: subscribe를 **3초 타임아웃**으로 걸어 3초 내면 반환 / 초과 시 `202 + task_id` 폴백하면 NFR도 챙긴다.
...
- **선택(시간 되면)**: 중복(따닥) 방지(`SETNX task_lock:{record_id}`), 워커 크래시 복구(`BRPOPLPUSH` 처리중 리스트)
